...
st.write("Настройка сцены: частота и объекты (rect/circle)")
...
for i in range(num_objects):
    ...
    shape = st.selectbox(f"Shape_{i}", ["rect", "circle"], index=0)
    ...
    if shape == "rect":
        ...
        col1, col2 = st.columns(2)
        with col1:
            cx = st.number_input(f"cx_{i}", value=1.5)
            cy = st.number_input(f"cy_{i}", value=1.0)
            r  = st.number_input(f"r_{i}",  value=0.3, min_value=0.0)
        with col2:
            material = st.selectbox(f"Материал_{i}", materials)
